  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/_health' || url.pathname === '/health') {
      return cors(origin, Response.json({
        status: 'ok', service: 'grudge-asset-cdn', version: '2.0.0',
    ...
    if (!key) {
      return cors(origin, Response.json({
        service: 'grudge-asset-cdn', version: '2.0.0',
        usage: 'GET /<path>',
...
async function serveAsset(request, url, key, env, ctx) {
  ...
  const gh = await fetch(`${GITHUB_PAGES_BASE}/${key}`, {
    headers: { 'User-Agent': 'GrudgeAssetCDN/2.0' },
  });
...
function buildHeaders(mime, cc, tags, etag, src, key) {
  const h = new Headers({ 'Content-Type': mime, 'Cache-Control': cc, 'X-Asset-Source': src, 'X-Powered-By': 'Grudge CDN/2.0' });
...
function cors(origin, resp) {
  ...
  h.set('Access-Control-Allow-Origin', ALLOWED_ORIGINS.has(origin) ? origin : 'https://grudgewarlords.com');
