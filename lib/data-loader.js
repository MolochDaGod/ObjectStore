...
const CACHE = {};
// Remote precedence: CF ObjectStore Worker (primary SSOT) -> CF Assets CDN (binary fallback) ->
// local /data (build-time prefetch). GitHub Pages removed from production remotes (legacy only).
// Fetch uses cache: 'no-store' so a previously cached Worker/R2 JSON copy cannot stick in the browser.
const REMOTES = [
