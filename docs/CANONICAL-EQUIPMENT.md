...
Pattern: `five-slot-unique` · Off-hand SHIELD/TOME: F-toggle injects slots 1–3. Anim clips: `sword_shield` / `longbow` / `magic` / `pistol` / `rifle` / `unarmed` pack roles. Class trees stay in `master-skillTrees.json`.
