...
const t0Weapons = T0_WEAPON_DEFS.map(w => ({
  ...
  iconUrl: `https://molochdagod.github.io/ObjectStore/icons/${w.icon}`,
  description: w.desc,
  stats: w.stats,
...
for (const mat of ARMOR_MATERIALS) {
  for (const part of ARMOR_PARTS) {
    ...
    t0Armor.push({
      ...
      tierLabel: TIER_LABEL_T0,
      tierColor: TIER_COLOR_T0,
      iconUrl: `https://molochdagod.github.io/ObjectStore/icons/${mat.icon(part.slot)}`,
...
const t0Consumables = T0_CONSUMABLES.map(c => {
  ...
  return {
    ...
    tierLabel: TIER_LABEL_T0,
    tierColor: TIER_COLOR_T0,
    iconUrl: `https://molochdagod.github.io/ObjectStore/icons/${c.icon}`,
    emoji: c.type === 'potion' ? '🧪' : '🍖',
    description: c.desc,
    effect: c.effect,
    stats: c.buff,
...
const t0Materials = T0_NEW_MATERIALS.map(m => ({
  ...
  iconUrl: `https://molochdagod.github.io/ObjectStore/icons/materials/${m.icon}`,
  description: m.desc,
}));
