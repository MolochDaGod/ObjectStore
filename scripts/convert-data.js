...
// ─── 2. Build equipment.json ───
const equipItems = parseEquipment();
console.log(`Parsed ${equipItems.length} equipment items from source`);
...
const equipJSON = {
  ...
  tierColors: {
    ...
    5: { name: 'Legendary', color: '#f59e0b' },
    6: { name: 'Mythic', color: '#ef4444' },
    7: { name: 'Eternal', color: '#ec4899' },
    8: { name: 'Divine', color: '#fbbf24' },
  },
  items: {},
};
...
for (const item of equipItems) {
  ...
  if (!equipJSON.items[matKey]) equipJSON.items[matKey] = [];

  // Add sprite path
  const spriteKey = item.id;
  if (sprites.armor[spriteKey]) {
    const slotFolder = SLOT_FOLDERS[item.type] || item.type.toLowerCase();
    item.spritePath = `/icons/armor/${slotFolder}/${sprites.armor[spriteKey]}.png`;
  }
  item.grudgeType = 'equipment';
  equipJSON.items[matKey].push(item);
}

fs.writeFileSync(path.join(OUT, 'equipment.json'), JSON.stringify(equipJSON, null, 2));
console.log(`✅ equipment.json: ${equipItems.length} items (${Object.keys(equipJSON.items).map(k => `${k}: ${equipJSON.items[k].length}`).join(', ')})`);

// ─── 3. Build spriteMaps.json ───
